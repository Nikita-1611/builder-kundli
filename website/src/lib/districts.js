...
// Every district a builder has warrants in, with their warrant count
// there, sorted highest first. A builder can only ever be linked to a
// district through this builders[] list (see sync-builders.mjs) -- there's
// no district field on builders/*.json itself.
export function findBuilderDistricts(districtsData, builderId) {
  ...
  for (const [district, entry] of Object.entries(districtsData || {})) {
    ...
    if (match) rows.push({ district, count: match.count });
  }
